...
let s:env_vars = "MAKEOBJDIRPREFIX=".shellescape($MAKEOBJDIRPREFIX)." GTAGSLIBPATH=".shellescape($GTAGSLIBPATH)
...
"
function! s:TrimOption(option, isfile)
    let l:option = ''
    let l:length = strlen(a:option)
    let l:i = 0
    let l:regex = a:isfile == 1 ? '[cenpquv]' : '[enpquv]'
...
function! s:ExecLoad(option, long_option, pattern)
    ...
    let l:option = l:option . s:TrimOption(a:option, l:isfile)
    if l:isfile == 1
        let l:cmd = s:env_vars . ' ' . s:global_command . ' ' . l:option . ' ' . g:Gtags_Shell_Quote_Char . a:pattern . g:Gtags_Shell_Quote_Char
    else
        let l:cmd = s:env_vars . ' ' . s:global_command . ' ' . l:option . 'e ' . g:Gtags_Shell_Quote_Char . a:pattern . g:Gtags_Shell_Quote_Char 
    endif
